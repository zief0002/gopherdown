...
This project is a modified version Chester Ismay's [thesisdown](https://github.com/ismayc/thesisdown) package to provide support for the University of Minnesota's thesis.Currently, the PDF version is fully functional. All other versions are derived from thesisdown and are not guaranteed to work.

An example of using this can be found [here](https://github.com/zief0002/dissertation), where I re-created my own PhD thesis.
...
Rather than installing a large TeX distribution, I recommend installing LaTeX via the `install_tinytex()` function from the R package [`{tinytex}`](https://yihui.org/tinytex/). 
...
To use `{gopherdown}`, open Rstudio, click on **File > New File > Rmarkdown ...** and then
select the **University of Minnesota Thesis** from the **Templates**.
